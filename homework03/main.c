...
void convert_coo_to_csr(int* row_ind, int* col_ind, double* val, 
                        ...
                        double** csr_vals)
{
    // Allocate row pointer, col, and val arrays
    *csr_row_ptr = malloc(sizeof(unsigned int) * m);
    *csr_col_ind = malloc(sizeof(unsigned int) * nnz);
    *csr_vals = malloc(sizeof(double) * nnz);

    // fill row pointer array with 0s
    for (int i=0; i<m; i++) {
        csr_row_ptr[0][i] = 0;
    }
    // increment vals in row pointer array for each non-zero number
    for (int i=0; i<nnz; i++) {
        int cur_ind = row_ind[i]-1;
        if (cur_ind != m)
            csr_row_ptr[0][cur_ind]++;
    }
    // each val = sum of all previous vals
    for (int i=1; i<m; i++) {
        csr_row_ptr[0][i] += csr_row_ptr[0][i-1];
    }

    // create copy of row pointer array
    unsigned int *temp_row_ptr = malloc(sizeof(unsigned int)*m);
    for (int i=0; i<m; i++) {
        temp_row_ptr[i] = csr_row_ptr[0][i];
        // printf("temp_row_ptr[%d] = %d\n", i, temp_row_ptr[i]);
    }
    // use copy of row pointer array to set row and val arrays
    for (int i=0; i<nnz; i++) {
        int cur_row = row_ind[i];
        int offset = temp_row_ptr[cur_row-1] - 1;
        csr_col_ind[0][offset] = (unsigned int) col_ind[i];
        csr_vals[0][offset] = (double) val[i];
        temp_row_ptr[cur_row-1]--;
        // printf("\nrow: %d, csr_col_ind: %u, csr_val: %f", cur_row, csr_col_ind[0][offset], csr_vals[0][offset]);
    }
    free(temp_row_ptr);
}
...
{
    // fill res array with 0s
    int start = 0;
    for (int i=0; i<m; i++) {
        res[i] = 0;
    }
    
    // implement spmv alg from slides
    for (int i=0; i<m; i++) {
        unsigned int end = csr_row_ptr[i];
        for (; start<end; start++) {
            int ind = start;
            double val = csr_vals[ind] * vector_x[csr_col_ind[ind]-1];
            res[i] += val;
        }
    }
}
