...
int dequeue(Queue *q)
{
    ...
    int goodbye = q->vals[q->front_i];
    q->front_i = (q->front_i+1)%1024;
    q->num_vals--;
    return goodbye;
}
         ...
         int* color, int* distance, int* parent)
{
    ...
    // init queue to track bfs
    Queue index_queue;
    initialize_queue(&index_queue);
    enqueue(&index_queue, source);

    // implement bfs algorithm
    while (front(&index_queue) != -1) {
        int u = dequeue(&index_queue);
        adj_node_t *cur_node = list[u];
        adj_node_t *adj_node = cur_node;
        while (adj_node != NULL) {
            if (color[adj_node->vid] == 0) {
                color[adj_node->vid] = 1;
                parent[adj_node->vid] = u;
                distance[adj_node->vid] = distance[u] + 1;
                enqueue(&index_queue, adj_node->vid);
            }
