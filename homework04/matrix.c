              ...
              int* color, int* distance)
{
    ...
    while(!done) {
        ...
        for (int i=0; i<rows; i++) {
            dst[i] = 0;
            for (int j=0; j<cols; j++) {
                // only modification is color check before taking dot product
                if (mat_trans[i][j]==1 && color[i]==0) {
                    int val = mat_trans[i][j]*src[j];
                    dst[i] += val;
                    // if value was added to dot product color node
                    if (val!=0) {
                        color[i] = 2;
                    }
                }
        ...
        // set distance and check for empty vector (base case)
        done = 1;
        for (int i=0; i<rows; i++) {
            if (dst[i]!=0) {
                distance[i] = iter;
